...
void mutation()
{
	for (int i = 0; i < N; i++) {
		...
		{
			do {
				//Note: This is the case for single point mutation
				int index_mutation = (rand() + time(0)) % (2 * num_units) + 1;
				int value_mutation = -1;
				do { value_mutation = (rand() + time(0)) % num_units; } while (value_mutation == item_array[i].circuit[index_mutation] || value_mutation == (index_mutation + 1) / 2 - 1);
				item_array[i].circuit[index_mutation] = value_mutation;
			} while (!"valid");//////////////////////still need valid function
		}
	}
}
